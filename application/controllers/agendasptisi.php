<?php
class agendasptisi extends CI_Controller {
	function index()
	{
		...
		if ($this->session->userdata('yangmasuk'))
		{
			...
			$data['menuisi']=$this->agendasptisimodel->getMenuIsi();
			if ($this->input->post('cek'))
			{
				if (strcmp($this->input->post('cek'),"tambah")==0)
					$this->agendasptisimodel->tambah();
				else if (strcmp($this->input->post('cek'),"ubah")==0)
					$this->agendasptisimodel->ubah();
				else if (strcmp($this->input->post('cek'),"hapusgbr")==0)
					$this->agendasptisimodel->hapusgbr1();	
			}
			else
				$this->load->view('agendasptisiview',$data);
		}
		else
