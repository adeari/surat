<?php
class agendaskdetail extends CI_Controller {
	function index()
	{
		...
		$this->agendaskisimodel->isigambardulu($this->input->get('isi'));
		...
		a.no_kendali from agendask a left join agendask_dari b on a.noagenda=b.noagenda 
		...
		{
			$isi.="<table>";
			$isi.="<tr><td colspan=\"3\" align=\"center\">Agenda SK</td>";
			$isi.="<td rowspan=\"13\" valign=\"midle\"  width=\"30%\">";
			$qry="select namafile from agendaskextra where noagenda=".$db->escape($this->input->get('isi'))." and urut in (1,2);";
			$hasil1 =$db->query($qry);
		...
		$qry="select namafile from agendaskextra where noagenda=".$db->escape($this->input->get('isi'))." and urut not in (1,2);";
		$hasil1 =$db->query($qry);
		foreach ($hasil1->result() as $row1)
		{
			$isi.="<a href=\"#\" onClick=\"window.open('".
		...
		$data = array(
		    ...
		    'onClick' => "window.open('agendaskrepot?isi='+encodeURIComponent('".$this->input->get('isi')."'),'reportmasuk','left='+Math.ceil(screen.width/2-500)+',top='+
		Math.ceil(screen.height/2-300)+',menubar=1,scrollbars=1,resizable=1,width=1000,height=600');"
		);
