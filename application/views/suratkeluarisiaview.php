...
<center><h1><font color="blue">Data Surat Keluar</font></h1></center>
		...
		new Ajax.Request(url, {
			...
			onSuccess: function(transport) {
				if (transport.responseText=='y')
				{
					...
					$('isiform').textcari.value=kunci;
					$('isiform').pilihcari.value='nosurat';
					$('isiform').action="suratkeluarlista/doupload";
					masukkankelist();
					$('isiform').submit();
				} else if (transport.responseText=='Nomor Kendali sudah ada silahkan menambah atau mengubah Nomor kendali')
				{
...
function pilihuser()
{
	window.open ("<?=$lincss;?>pilihpenerima?ii=suratkeluar&uiu="+$('isiform').pilihankita.value,"mywindow","left="+Math.ceil(screen.width/2-133)+",top="+
