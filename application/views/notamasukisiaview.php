		...
		new Ajax.Request(url, {
			...
			onSuccess: function(transport) {
				if (transport.responseText=='y')
				{
					...
					$('isiform').textcari.value=kunci;
					$('isiform').pilihcari.value='nonota';
					$('isiform').action="notamasuklista/doupload";
					masukkankelist();
					$('isiform').submit();
				} else alert(transport.responseText);
			}
...
function pilihuser()
{
	window.open ("<?=$lincss;?>pilihpenerimanota?ii=notamasuk&uiu="+$('isiform').pilihankita.value,"mywindow","left="+Math.ceil(screen.width/2-133)+",top="+
