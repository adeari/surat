...
<center><h1><font color="blue">Data Agenda SPT</font></h1></center>
...
function tambah(){
	var kunci=$F('txnoagenda').gsub(' ', '');
	if (kunci.length<1) {
		alert('Nomor Agenda tidak boleh KOSONG...');
	} else {
		var url="agendasptisi";
		new Ajax.Request(url, {
			...
			onSuccess: function(transport) {
				if (transport.responseText=='y')
				{
					...
					$('isiform').pilihcari.value='noagenda';
...
function ubah(){
	...
	if (kunci.length<1) {
		...
		new Ajax.Request(url, {
			...
			no_rak:$F('txno_rak'),
			no_baris:$F('txno_baris'),
			noagendaubah:$('isiform').pilihankita.value,
			no_kendali:$F('txno_kendali'),
			no_berkas:$F('txno_berkas')
			},
			onSuccess: function(transport) {
				if (transport.responseText=='y')
				{
					...
					$('isiform').textcari.value=kunci;
					$('isiform').pilihcari.value='noagenda';
					$('isiform').action="agendasptlist/doupload";
					$('isiform').submit();
				} else if (transport.responseText=='Nomor Kendali sudah ada silahkan menambah atau mengubah Nomor kendali')
				{
					$('txno_kendali').focus();
