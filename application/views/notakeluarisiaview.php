...
<center><h1><font color="blue">Data Nota Keluar</font></h1></center>
		...
		new Ajax.Request(url, {
			...
			onSuccess: function(transport) {
				if (transport.responseText=='y')
				{
					...
					$('isiform').textcari.value=kunci;
					$('isiform').pilihcari.value='nonota';
					$('isiform').action="notakeluarlista/doupload";
					masukkankelist();
					$('isiform').submit();
				} else alert(transport.responseText);
			}
...
function pilihuser()
{
	window.open ("<?=$lincss;?>pilihpenerimanota?ii=notakeluar&uiu="+$('isiform').pilihankita.value,"mywindow","left="+Math.ceil(screen.width/2-133)+",top="+
