...
<form name="dataliat" id="dataliat" method="post" action="<?=$lincss;?>suratkeluarlista">
...
<center><h1><font color="blue">Surat Keluar</font></h1></center>
...
<table class="wp-list-table widefat posts" cellspacing="0">
	...
	<tr>
		...
		<th scope="col" id="author" class="manage-column column-author sortable desc"><a href="#" onClick="orderBy('nosurat')" >No.&nbsp;Surat</a></th>
		<th scope="col" id="author" class="manage-column column-author sortable desc" style="text-align:center"><a href="#" onClick="orderBy('no_rak')" >No.&nbspRak</a></th>
		<th scope="col" id="author" class="manage-column column-author sortable desc" style="text-align:center"><a href="#" onClick="orderBy('no_baris')" >No.&nbspBaris</a></th>
		<th scope="col" id="author" class="manage-column column-author sortable desc"><a href="#" onClick="orderBy('tanggal')" >Tanggal</a></th>
		<th scope="col" id="author" class="manage-column column-author sortable desc"><a href="#" onClick="orderBy('dari')" >Bagian / SUBDIN</a></th>
		<th scope="col" id="tags" class="manage-column column-tags"><a href="#" onClick="orderBy('tujuan')" >Tujuan</a></th>
		<th scope="col" id="tags" class="manage-column column-tags"><a href="#" onClick="orderBy('isi')" >Perihal</a></th>
	</tr>
	</thead>
	<tfoot>
	<tr>
...
<tr class="alternate" valign="top">
	<td rowspan="4"><div align="right"><font color="red" width="95%"><h3>Jumlah Surat Keluar</h3></font></div></td>
...
{
	$('dataliat').action='<?=$lincss;?>suratkeluarisia';
	$('dataliat').submit();
}
function cari() {
	$('dataliat').halaman.value='0';
...
function detail(isi) {
	window.open ("<?=$lincss;?>suratkeluardetaila?isi="+encodeURIComponent(isi),"mywindow","left="+Math.ceil(screen.width/2-500)+",top="+
		     Math.ceil(screen.height/2-300)+",scrollbars=1,menubar=1,resizable=1,width=1000,height=600");
}

function komentar(isi) {
	window.open ("<?=$lincss;?>komentarsuratkeluara?isi="+encodeURIComponent(isi),"komen","left="+Math.ceil(screen.width/2-500)+",top="+
		     Math.ceil(screen.height/2-300)+",scrollbars=1,menubar=1,resizable=1,width=1000,height=600");
}
function ubah(isi)
{
