<?
class suratkeluarlistamodel extends CI_Model {    
    ...
    function getTotalData()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(a.nosurat) as jumlah from keluar a left join keluar_dari b on a.nosurat=b.nosurat;";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatahariini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(a.nosurat) as jumlah from keluar a left join keluar_dari b on a.nosurat=b.nosurat  where date(a.tanggal)=date(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
	...
	if (strlen($this->input->post('textcari'))>0)
	{
		...
		||strcmp($this->input->post('pilihcari'),"tanggal")==0
		)
	...
	$qry="select a.nosurat as jumlah from keluar a left join keluar_dari b on a.nosurat=b.nosurat ".$this->getKondisiIsiData()." ".$limit.";";
	$hasil =$db->query($qry);
        ...
        if ($this->input->post('ordernya'))
	{
        ...
        $qry="select a.no_urut,a.nosurat,DATE_FORMAT(a.tanggal, '%d/%m/%Y') as  tanggalv ,a.dari,a.tujuan,a.isi,b.no_rak,b.no_baris from keluar a
	left join keluar_dari b on a.nosurat=b.nosurat ".
    ...
    function getViewHalaman()
    {
        ...
        $db=$this->db;
        $qry="select count(*) as jumlah from keluar a left join keluar_dari b on a.nosurat=b.nosurat  ".$this->getKondisiIsiData().";";
        $hasil =$db->query($qry);
        $row=$hasil->row();
	...
	$qry1="";
        ...
        $qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from keluar_dari ";
	$qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from keluar_ke ";
	$qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from keluar_komentar ";
	$qry.=$kondisihapus;
        $db->query($qry);
	...
	if (strcmp($this->input->post('bttambah1'),"tambah")==0)
	{
	    ...
	    $db->query($qry);
	    $qry="insert into keluar_dari (nosurat,userid,no_baris,no_rak) values (".$db->escape(str_replace(" ", "", $this->input->post('txnosurat'))).","
	...
	} else if (strcmp($this->input->post('bttambah1'),"ubah")==0)
	{
	    ...
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	    $qry="delete from keluar_dari  where nosurat=".$db->escape($this->input->post('pilihankita')).";";
	    $db->query($qry);
	    $qry="insert into keluar_dari (nosurat,userid,no_baris,no_rak) values (".$db->escape(str_replace(" ", "", $this->input->post('txnosurat'))).","
	    .$db->escape($this->session->userdata('yangmasuk')).",".$this->angkaSQL($this->input->post('txno_baris'))
	    .",".$this->angkaSQL($this->input->post('txno_rak')).");";
	    $db->query($qry);
	    $qry="update keluar_komentar set nosurat=".$db->escape(str_replace(" ", "", $this->input->post('txnosurat')))
	...
	if (strcmp($this->input->post('bttambah1'),"ubah")==0)
		$qry="delete from keluar_ke where nosurat=".$db->escape($this->input->post('pilihankita')).";";
	    else
		$qry="delete from keluar_ke where nosurat=".$db->escape(str_replace(" ", "", $this->input->post('txnosurat'))).";";
	    $db->query($qry);
	if (strlen($kirimkeisi[0])>0)
	{
	    ...
	    {
		$qry="insert into keluar_ke (nosurat,userid) values ("
		.$db->escape(str_replace(" ", "", $this->input->post('txnosurat'))).","
	...
	$db->query($qry);
	$qry="delete from keluar_dari where nosurat=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="delete from keluar_ke where nosurat=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="delete from keluar_komentar where nosurat=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="insert into tracking1 (tanggal,keterangan,nama_user)
