...
class suratmasukmodel extends CI_Model {    
    ...
    function getTotalData()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nosurat) as jumlah from masuk;";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatahariini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nosurat) as jumlah from masuk where date(tgl_masuk)=date(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatamingguini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nosurat) as jumlah from masuk where week(tgl_masuk)=week(now()) and year(tgl_masuk)=year(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatabulanini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nosurat) as jumlah from masuk where month(tgl_masuk)=month(now()) and year(tgl_masuk)=year(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
	...
	$qry="select nosurat as jumlah from masuk ".$this->getKondisiIsiData()." ".$limit.";";
	$hasil =$db->query($qry);
        ...
        if ($this->input->post('halaman'))
	{
        ...
        $qry="select no_urut,nosurat,DATE_FORMAT(tanggal, '%d/%m/%Y') as  tanggalv ,dari,tujuan,isi from masuk ".
    ...
    function getViewHalaman()
    {
        ...
        $db=$this->db;
        $qry="select count(*) as jumlah from masuk ".$this->getKondisiIsiData().";";
        $hasil =$db->query($qry);
        $row=$hasil->row();
