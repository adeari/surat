<?
class notakeluarlistmodel extends CI_Model {    
    ...
    function getTotalData()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nonota) as jumlah from nota_keluar;";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatahariini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nonota) as jumlah from nota_keluar where date(tanggal)=date(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatamingguini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nonota) as jumlah from nota_keluar where week(tanggal)=week(now()) and year(tanggal)=year(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getTotalDatabulanini()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(nonota) as jumlah from nota_keluar where month(tanggal)=month(now()) and year(tanggal)=year(now());";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
    ...
    function getKondisiIsiData()
    {
	    ...
	    &&strlen($this->input->post('textcari'))>0 )
		    ...
		    ||strcmp($this->input->post('pilihcari'),"tgl_pengiriman")==0
		)
		 &&strlen($this->input->post('textcari'))>0 )
	...
	$qry="select nonota as jumlah from nota_keluar ".$this->getKondisiIsiData()." ".$limit.";";
	$hasil =$db->query($qry);
        ...
        if ($this->input->post('halaman'))
	{
        ...
        $qry="select no_urut,nonota,DATE_FORMAT(tanggal, '%d/%m/%Y') as  tanggalv ,keterangan,tujuan,perihal from nota_keluar ".
    ...
    function getViewHalaman()
    {
        ...
        $config['base_url'] = 'notakeluarlist?';
        $db=$this->db;
        $qry="select count(*) as jumlah from nota_keluar ".$this->getKondisiIsiData().";";
        $hasil =$db->query($qry);
        $row=$hasil->row();
	...
	$qry1="";
		...
		values (now(),'Hapus Nota Keluar , No. Nota : ".$this->input->post("ck".$i)."','".$this->session->userdata('yangmasuk')."');";
		$db->query($qry1);
                if ($awal)
                {
	...
	if (strcmp($this->input->post('bttambah1'),"tambah")==0)
	{
	    ...
	    values (now(),'Tambah Nota Keluar, No. Nota : ".$db->escape_like_str(str_replace(" ", "", $this->input->post('txnonota')))
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	    $qry="insert into nota_keluar (no_urut,nonota,tanggal,tgl_pengiriman,tujuan,keterangan,sifat,perihal) values (423".
	...
	{
	    $qry="insert into tracking1 (tanggal,keterangan,nama_user)
	    values (now(),'Edit Nota Keluar, No. Nota : ".$db->escape_like_str(str_replace(" ", "", $this->input->post('txnonota')))
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	...
	$db->query($qry);
	$qry="insert into tracking1 (tanggal,keterangan,nama_user)
	    values (now(),'Hapus Nota Keluar , No. Surat : ".$this->input->post('bthapus1')."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
