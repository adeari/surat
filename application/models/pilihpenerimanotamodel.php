<?
class pilihpenerimanotamodel extends CI_Model {    
	...
	$valuenya=$this->input->get('ii');
			...
			if (strcmp($valuenya,"notamasuk")==0)
			$balik="txperihal";
	return $balik;
    }
    function pilihtabel()
	...
	if (strcmp($valuenya,"notamasuk")==0)
	    $balik="nota_masuk";
	else if (strcmp($valuenya,"notakeluar")==0)
	    $balik="nota_keluar";
	    return $balik;
    }
    function getJmlData()
	...
	$qry="select count(userid) as jumlah from ".$this->pilihtabel()."_ke where nonota=".$db->escape($valuenya)
