<?
class suratkeluarisimodel extends CI_Model {
	...
	$balik.=form_open_multipart('suratkeluarlist/doupload', $attributes);
	$nosuratv="";$txttanggalv=date('d/m/Y');$dariv="";$tujuanv="";$keteranganv="";$isiv="";
	$no_berkasv="";$no_kendaliv="";
	
	
	if ($this->input->post('pilihankita'))
	{
	    $pilihankita=$this->input->post('pilihankita');
	    $qry="select nosurat,DATE_FORMAT(tanggal, '%d/%m/%Y') as  tanggalv,dari,tujuan,keterangan, 
	    isi,no_berkas,no_kendali from keluar
	...
	$qry="select no_berkas,no_kendali from keluar where nosurat=".$db->escape($this->input->post("nosuratubah")).";";
	$hasil = $db->query($qry);
	$row = $hasil->row();
