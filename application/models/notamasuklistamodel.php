<?
class notamasuklistamodel extends CI_Model {    
    ...
    function getTotalData()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(a.nonota) as jumlah from nota_masuk a left join nota_masuk_dari b on a.nonota=b.nonota;";
        $hasil =$db->query($qry);
        $row=$hasil->row();
        $balik=$row->jumlah;
        return $balik;
	...
	$qry="select a.nonota as jumlah from nota_masuk  a  left join nota_masuk_dari b on a.nonota=b.nonota
        ...
        if ($this->input->post('ordernya'))
	{
        ...
        $qry="select a.no_urut,a.nonota,DATE_FORMAT(a.tanggal, '%d/%m/%Y') as  tanggalv ,a.dari,a.tujuan,a.isi,b.no_rak,b.no_baris 
	from nota_masuk  a  left join nota_masuk_dari b on a.nonota=b.nonota ".
    ...
    function getViewHalaman()
    {
        ...
        $config['base_url'] = 'notamasuklista?';
        $db=$this->db;
        $qry="select count(*) as jumlah from nota_masuk a  left join nota_masuk_dari b on a.nonota=b.nonota ".$this->getKondisiIsiData().";";
        $hasil =$db->query($qry);
        $row=$hasil->row();
	...
	$qry1="";
        ...
        $qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from nota_masuk_dari ";
	$qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from nota_masuk_ke ";
	$qry.=$kondisihapus;
        $db->query($qry);
	$qry=" delete from nota_masuk_komentar ";
	$qry.=$kondisihapus;
        $db->query($qry);
	...
	if (strcmp($this->input->post('bttambah1'),"tambah")==0)
	{
	    ...
	    $db->query($qry);
	    $qry="insert into nota_masuk_dari (nonota,userid,no_baris,no_rak)
	...
	} else if (strcmp($this->input->post('bttambah1'),"ubah")==0)
	{
	    ...
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	    $qry="delete from nota_masuk_dari where nonota=".$db->escape($this->input->post('pilihankita')).";";
	    $db->query($qry);
	    $qry="insert into nota_masuk_dari (nonota,userid,no_baris,no_rak)
	    values (".$db->escape(str_replace(" ", "", $this->input->post('txnonota'))).","
	    .$db->escape($this->session->userdata('yangmasuk')).",".$this->angkaSQL($this->input->post('txno_baris'))
	    .",".$this->angkaSQL($this->input->post('txno_rak')).");";
	    $db->query($qry);
	    $qry="update nota_masuk_komentar set nonota=".$db->escape(str_replace(" ", "", $this->input->post('txnonota')))
	...
	if (strcmp($this->input->post('bttambah1'),"ubah")==0)
		$qry="delete from nota_masuk_ke where nonota=".$db->escape($this->input->post('pilihankita')).";";
	    else
		$qry="delete from nota_masuk_ke where nonota=".$db->escape(str_replace(" ", "", $this->input->post('txnonota'))).";";
	    $db->query($qry);
	if (strlen($kirimkeisi[0])>0)
	{
	    ...
	    {
		$qry="insert into nota_masuk_ke (nonota,userid) values ("
		.$db->escape(str_replace(" ", "", $this->input->post('txnonota'))).","
	...
	$db->query($qry);
	$qry="delete from nota_masuk_dari where nonota=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="delete from nota_masuk_ke where nonota=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="delete from nota_masuk_komentar where nonota=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="insert into tracking1 (tanggal,keterangan,nama_user)
