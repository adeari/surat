<?
class agendasklistmodel extends CI_Model {    
    ...
    function getTotalData()
    {
        $balik="";
        $db=$this->db;
        $qry="select count(a.noagenda) as jumlah from agendask a
	...
	$qry="select a.noagenda as jumlah from agendask a
	left join agendask_dari b on a.noagenda=b.noagenda
        ...
        if ($this->input->post('ordernya'))
	{
        ...
        $qry="select a.no_urut,a.noagenda,DATE_FORMAT(a.tanggal, '%d/%m/%Y') as  tanggalv ,a.dari,a.tujuan,a.keterangan,b.no_rak,b.no_baris 
	from agendask a
	left join agendask_dari b on a.noagenda=b.noagenda ".
    ...
    function getViewHalaman()
    {
        ...
        $config['base_url'] = 'agendasklist?';
        $db=$this->db;
        $qry="select count(*) as jumlah from agendask a
	left join agendask_dari b on a.noagenda=b.noagenda
	 ".$this->getKondisiIsiData().";";
        $hasil =$db->query($qry);
        $row=$hasil->row();
	...
	$qry1="";
		...
		values (now(),'Hapus Agenda SK , No. Agenda : ".$this->input->post("ck".$i)."','".$this->session->userdata('yangmasuk')."');";
		$db->query($qry1);
                if ($awal)
                {
        ...
        $qry.=$kondisihapus;
        $db->query($qry);
	$qry="select namafile from agendaskextra ".$kondisihapus;
	$hasil =$db->query($qry);
	...
	$qry=" delete from agendaskextra ".$kondisihapus;
	$db->query($qry);
    }
	...
	if (strcmp($this->input->post('bttambah1'),"tambah")==0)
	{
	    ...
	    values (now(),'Tambah Agenda SK , No. Agenda : ".$db->escape_like_str(str_replace(" ", "", $this->input->post('txnoagenda')))
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	    $qry="insert into agendask_dari (noagenda,no_rak,no_baris) 
	    ...
	    $db->query($qry);
	    $qry="insert into agendask (no_urut,noagenda,tanggal,dari,tujuan,keterangan,isi,no_berkas
	...
	{
	    $qry="insert into tracking1 (tanggal,keterangan,nama_user)
	    values (now(),'Edit Agenda SK , No. Agenda : ".$db->escape_like_str(str_replace(" ", "", $this->input->post('txnoagenda')))
	    ."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	    $qry="delete from agendask_dari where noagenda=".$db->escape($this->input->post('pilihankita')).";";
	    $db->query($qry);
	    $qry="insert into agendask_dari (noagenda,no_rak,no_baris) 
	    values (".$db->escape(str_replace(" ", "", $this->input->post('txnoagenda')))
	    .",".$this->angkaSQL($this->input->post('txno_rak')).",".$this->angkaSQL($this->input->post('txno_baris')).");";
	    $db->query($qry);
	    $qry="update agendask set noagenda=".$db->escape(str_replace(" ", "", $this->input->post('txnoagenda'))).
	...
	{
	    $qry="update agendaskextra set noagenda=".$db->escape(str_replace(" ", "", $this->input->post('txnoagenda')))
	    ." where noagenda=".$db->escape($this->input->post('pilihankita')).";";
	    $db->query($qry);
	}
	for ($i=1;$i<11;$i++)
	{
	    ...
	    {
		$namafileupload="gbrask".$nomorpostfile.$this->ambilnomernya($this->input->post('txnoagenda')).
		$this->session->userdata('yangmasuk').".jpg";
		move_uploaded_file($_FILES[$namapostfile]['tmp_name'], $direktoriuplod.$namafileupload);
		$qry="select namafile from agendaskextra where urut=".$nomorpostfile." 
		...
		$qry="delete from agendaskextra where urut=".$nomorpostfile." 
		and noagenda=".$db->escape(str_replace(" ", "", $this->input->post('txnoagenda')))
		.";";
		$db->query($qry);
    ...
    {
	$db=$this->db;
	$qry="delete from agendask where noagenda=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
	$qry="insert into tracking1 (tanggal,keterangan,nama_user)
	    values (now(),'Hapus Agenda SK , No. Agenda : ".$this->input->post('bthapus1')."','".$this->session->userdata('yangmasuk')."');";
	    $db->query($qry);
	$qry="select namafile from agendaskextra where noagenda=".$db->escape($this->input->post('bthapus1')).";";
	$hasil =$db->query($qry);
	$direktoriuplod="gambaruplod/";
        foreach ($hasil->result() as $row)
        {
	...
	$qry="delete from agendaskextra where noagenda=".$db->escape($this->input->post('bthapus1')).";";
	$db->query($qry);
    }
