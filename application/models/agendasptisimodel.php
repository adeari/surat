<?
class agendasptisimodel extends CI_Model {
	...
	{
	    $qry="select namafile from agendasptextra  where urut=".$urut." 
    ...
    {
	$db=$this->db;
	$qry="SELECT no_kendali AS jumlah FROM agendaspt WHERE no_kendali REGEXP '^[0-9]+$'
	AND length( no_kendali ) = (SELECT max( length( no_kendali ) ) AS jumlah FROM agendaspt WHERE no_kendali REGEXP '^[0-9]+$' ) ";
	    $hasil =$db->query($qry);
	    $nomor=0;
    ...
    {
	$db=$this->db;
	$qry="select no_berkas as jumlah from agendaspt where no_berkas REGEXP '^[0-9]+$'
	AND length( no_berkas ) = (SELECT max( length( no_berkas ) ) AS jumlah FROM agendaspt WHERE no_berkas REGEXP '^[0-9]+$' ) ";
	    $hasil =$db->query($qry);
	    $nomor=0;
	    foreach ($hasil->result() as $row)
	    {
	...
	$balik.=form_open_multipart('agendasptlist/doupload', $attributes);
	$noagendav="";$txttanggalv=date('d/m/Y');$dariv="";$tujuanv="";$keteranganv="";$isiv="";
	$no_berkasv="";$no_kendaliv="";$no_rakv="";$no_barisv="";
	...
	if ($this->input->post('pilihankita'))
	{
	    ...
	    a.isi,a.no_berkas,a.no_kendali,b.no_rak,b.no_baris from agendaspt a left join agendaspt_dari b on a.noagenda=b.noagenda 
	...
	$qry="select count(*) as jumlah from agendaspt where noagenda=".$db->escape($this->input->post("noagenda")).";";
	$hasil = $db->query($qry);
	    $row = $hasil->row();
	    if ($row->jumlah>0) {
		echo "Nomor Agenda ini sudah ada";
		$bisa=false;
	    }
	if ($bisa&&strlen($this->input->post("no_berkas"))>0)
	{
		$qry="select count(*) as jumlah from agendaspt where no_berkas=".$db->escape($this->input->post("no_berkas")).";";
		$hasil = $db->query($qry);
		$row = $hasil->row();
		if ($row->jumlah>0) {
	...
	{
		$qry="select count(*) as jumlah from agendaspt where no_kendali=".$db->escape($this->input->post("no_kendali")).";";
		$hasil = $db->query($qry);
		$row = $hasil->row();
		if ($row->jumlah>0) {
	...
	{
		$qry="select count(*) as jumlah from agendaspt_dari where no_rak=".
	...
	$qry="select a.no_berkas,a.no_kendali,b.no_rak,b.no_baris from agendaspt a
    ...
    {
	$db=$this->db;
	$qry="select namafile from agendasptextra where
	...
	$qry="delete from agendasptextra where  noagenda=".$db->escape($this->input->post("noagenda"))." and urut=".$this->input->post("nomorgambar").";";
	echo $qry;
	if ($db->query($qry))
	    echo "y";
    }
