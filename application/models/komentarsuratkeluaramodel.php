<?
class komentarsuratkeluaramodel extends CI_Model {
    ...
    {
	$db=$this->db;
	$qry="delete from keluar_komentar where nosurat=".$db->escape($this->input->get('isi'))." and nourut=".$this->input->post('urut');
	$db->query($qry);
    }
	...
	$db=$this->db;
	$balik=false;
	$qry="select count(*) as jumlah from keluar_dari where nosurat=".$db->escape($this->input->get('isi'))
	...
	if ($row->jumlah>0)
	    ...
	    $qry="select count(*) as jumlah from keluar_komentar where nosurat=".$db->escape($this->input->get('isi'))
	...
	$qry="select komentar,nourut,userid  from keluar_komentar where nosurat=".$db->escape($nosurat)." order by nourut";
	$hasil = $db->query($qry);
	$ganti=true;
	$klas="";
	$balik="";
    ...
    {
	$db=$this->db;
	$qry="select (max(nourut)+1) as jumlah from keluar_komentar where nosurat=".$db->escape($this->input->get('isi'));
	$hasil = $db->query($qry);
	$row = $hasil->row();
	if ($row->jumlah==null)
	    $urutannya=1;
	...
	$qry="insert into keluar_komentar (nosurat,nourut,userid,komentar,jam) values (".$db->escape($this->input->get('isi'))."
	,".$urutannya."
	...
	$balik="";
	$db=$this->db;
	$qry="select isi,keterangan from keluar where nosurat=".$db->escape($this->input->get('isi'));
	$hasil = $db->query($qry);
	$row = $hasil->row();
